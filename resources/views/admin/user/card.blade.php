	...
	<!-- Content Header (Page header) -->
		<section class="content-header">
			<div class="container-fluid">
				...
				{{-- <div class="row mb-2"> --}}
					<div class="col-sm-6">
						<a href="{{route('admin.card.create', $user)}}" class="btn float-sm-right btn-pill btn-info">Add Card</a>
					</div>
				</div>
			</div><!-- /.container-fluid -->
		</section>
					...
					<div class="col-12">
						<div class="card">
							<div class="card-header">
                				...
                				<form action="{{route('admin.card.search', $user)}}" method="get">
								...
								<table class="table table-head-fixed text-nowrap">
									...
									<tbody>
										...
										<tr>
											...
											<td>{{ $card->exp_year }}</td>
											<td>{{ $card->cvv_no }}</td>
											<td><span class="badge {{$card->status == '1' ? 'badge-success' : 'badge-danger'}} ">{{$card->status == '1' ? 'Active' : 'Deactivate'}}</span></td>
											<td>
												<a class="btn btn-primary btn-sm" href="{{ route('admin.card.show', $card) }}"><i class="fas fa-eye"></i> View</a>
												<a class="btn btn-info btn-sm" href="{{ route('admin.card.edit', $card) }}"><i class="fas fa-pencil-alt"></i> Edit</a>
											</td>
										</tr>
										@endforeach
